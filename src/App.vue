...
<v-app>
    <div>
        <v-app-bar app color="white" flat v-if="isLogin" elevate-on-scroll>
            ...
            <v-btn depressed text color="white" to="/" class=" d-flex align-center jusify">
                <v-img alt="Brass & Books" class="shrink mx-2 ml-sm-0" contain src="./assets/logo.png" transition="scale-transition" width="40" />
                <span class="primary--text font-weight-medium text-h6">BB-DMS</span>
            </v-btn>

            <v-btn class="d-none d-sm-flex" text color="primary" to="/" exact-active-class="secondary">
                Investors
            </v-btn>
            <v-btn class="d-none d-sm-flex" text color="primary" to="/investments" exact-active-class="secondary">
                Investments
            </v-btn>
            <v-spacer> </v-spacer>
        ...
        <v-navigation-drawer v-model="drawer" dark  app color="primary">
            ...
            <v-list dark color="primary">
                ...
                <v-list-item v-for="link in sideLinks" :key="link.name" :to="link.route" active-class="secondary--text" exact link nav @click="switchPage(link.name)">
                    <v-list-item-icon>
