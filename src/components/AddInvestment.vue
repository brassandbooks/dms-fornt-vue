                    ...
                    <v-card-text>
                        <v-container>
                            <v-form @submit.prevent="save" ref="form" v-model="valid" :lazy-validation="lazy">
                                <v-row no-gutters>
                                    <v-col cols="12">
                                        <v-text-field :rules="[v => !!v || 'Principal Sum is required']" type="text" v-model="principalSum" label="Principal Sum"></v-text-field>
                                    </v-col>
                                    <v-col cols="12">
                                        <v-select :rules="[v => !!v || 'Product is required']" :items="products" v-model="product" label="Products"></v-select>
                                    </v-col>
                                    <v-col cols="12">
                                        <v-text-field :rules="[v => !!v || 'Interest Rate is required']" type="text" v-model="interestRate" label="Interest Rate"></v-text-field>
                                    </v-col>
                                    <v-col cols="12">
                                        <v-menu ref="menu" v-model="menu"  :close-on-content-click="false" transition="scale-transition" offset-y min-width="290px">
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-text-field :rules="[v => !!v || 'Effective Date is required']" v-model="effectiveDate" label="Effective Date" prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                                            </template>
                                    ...
                                    <v-col cols="12">
                                        <v-text-field :rules="[v => !!v || 'Investment Duration is required']" type="text" v-model="investmentDuration" label="Investment Duration"></v-text-field>
                                    </v-col>
                                    <v-col cols="12">
                                        <v-select :rules="[v => !!v || 'Distribution Date is required']" :items="distributionDates" v-model="distributionDate" label="Distribution Date"></v-select>
                                    </v-col>
                                    <v-col cols="12">
                                        <v-select :rules="[v => !!v || 'Payout Frequency is required']" :items="payoutFrequencys" v-model="payoutFrequency" label="Payout Frequency"></v-select>
                                    </v-col>

                                    <v-col cols="12" class="d-flex justify-end">
                                        <v-spacer></v-spacer>
                                        <v-btn color="primary" text @click="close">Cancel</v-btn>
                                        <v-btn color="primary" :loading="loading.addInvestment" depressed type="submit">Add Investment</v-btn>
                                    </v-col>
                                </v-row>
                            </v-form>
                        </v-container>
